                            ...
                            <tbody className="divide-y divide-slate-100">
                                {students.length > 0 ? students.map((student) => (
                                    <tr key={student.id} className="text-slate-700">
                                        ...
                                        <td className="px-3 py-4">{student.class || 'Unassigned'}</td>
                                        <td className="px-3 py-4">{student.branch || 'Main school'}</td>
                        ...
                        <div className="mt-5 space-y-3">
                            {parents.length > 0 ? parents.map((parent) => (
                                <div key={parent.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                    ...
                                    <div className="mt-1 text-xs text-slate-500">{parent.relationship || 'Guardian'} • {parent.phone || 'No phone'}{parent.email ? ` • ${parent.email}` : ''}</div>
                                </div>
